...
export default function Sidebar({ menuItems, title, title_menu }) {
  ...
  return (
    <div className="relative w-0 md:w-1/5 mt-[4.5rem] md:px-6 px-0 block md:sticky h-[calc(100vh-55px)]">
      <h1 className="text-3xl font-bold md:block hidden w-full mb-4 mt-12">{title}</h1>
      ...
      <div
        ref={menuRef}
        className={`md:block z-30 md:visible md:opacity-100 ${
          showMenu ? 'visibility py-10 duration-300 opacity-100 absolute border-2 border-black rounded-md left-10 top-20 w-32 bg-white' : 'invisible left-[120%] top-[120%] w-full opacity-0'
        }`}
      >
