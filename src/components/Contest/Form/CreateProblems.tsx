...
export default function CreateProblem({ contestId }) {
      ...
      const fetchData = async () => {
        try {
          ...
          setOriginalSelectedCategories(
            data.categories.map((category: { id: any }) => category.id),
          );
          ...
          setSampleInputs(
            Array.from({ length: maxLines + 1 }, (_, i) => inputLines[i] || ""),
          );
          setSampleOutputs(
            Array.from(
              { length: maxLines + 1 },
          ...
          console.error(
            `Error fetching problem data with ID ${editingProblemsId}`,
            error,
          );
        }
      };
  ...
  useEffect(() => {
    const fetchTestCase = async () => {
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/test_cases`,
        );
        const data = await response.json();
        setTestCase(data);
      } catch (error) {
  ...
  const handleCreateProblem = async () => {
    ...
    try {
      const problemResponse = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/contest/problem`,
        {
          method: "POST",
          headers: {
      ...
      if (problemResponse.status === 200) {
        ...
        for (const categoryId of selectedCategories) {
          ...
          if (addCategoryResponse.status === 200) {
            console.log(
              `Category ${categoryId} added successfully to problem ${newProblemId}`,
            );
          } else {
            const errorData = await addCategoryResponse.json();
            console.log(
              `Failed to add category. Server error: ${JSON.stringify(errorData)}`,
            );
          }
        }
        router.push(
          `/admin/problems/create/testcase?problem_id=${newProblemId}`,
        );
      } else {
        const errorData = await problemResponse.json();
        console.log(
          `Failed to create problem. Server error: ${JSON.stringify(errorData)}`,
        );
      }
    } catch (error) {
      ...
      if (response.status === 200) {
        ...
        for (const categoryId of selectedCategories) {
          if (!originalSelectedCategories.includes(categoryId)) {
            ...
            if (addCategoryResponse.status === 200) {
              console.log(
                `Category ${categoryId} added successfully to problem ${editingProblemsId}`,
              );
            } else {
              const errorData = await addCategoryResponse.json();
              console.log(
        ...
        for (const categoryId of categoriesToBeRemoved) {
          ...
          if (removeCategoryResponse.status === 200) {
            console.log(
              `Category ${categoryId} removed successfully from problem ${editingProblemsId}`,
            );
          } else {
            const errorData = await removeCategoryResponse.json();
            console.log(
              `Failed to remove category. Server error: ${JSON.stringify(errorData)}`,
            );
          }
        }
        ...
        if (matchingTestCase) {
          ...
          router.push(
            `/admin/problems/edit/${editingProblemsId}/testcase/${correspondingTestCaseId}`,
          );
        } else {
          router.push(
            `/admin/problems/create/testcase?problem_id=${editingProblemsId}`,
          );
        }
      } else {
    ...
    <div className="mt-20">
      <h1 className="text-3xl font-bold lg:text-4xl">
        <span className="underline decoration-blue-500">
          {editedProblemsData.id ? "Edit Problem" : "Create Problem"}
        </span>
      </h1>
      <form action="" className="">
        ...
        <div className=" mb-3">
          ...
          <div className="mb-2 mt-1 grid grid-cols-3">
        ...
        <div className="mb-4">
          <label className="mb-1 block">Time Limit</label>
          <input
            type="number"
            name=""
            id=""
        ...
        <div className="mb-4">
          <label className="mb-1 block">Memory Limit</label>
          <input
            type="number"
            placeholder="0"
            name=""
          ...
          />
          {memoryLimitError && (
            <p className="text-red-500">{memoryLimitError}</p>
          )}
        </div>
        <div className="mb-4">
          <label className="mb-1 block">Input Format</label>
          <input
            type="text"
            name=""
            id=""
          ...
          />
          {inputFormatError && (
            <p className="text-red-500">{inputFormatError}</p>
          )}
        </div>
        <div className="mb-4">
          ...
          />
          {outputFormatError && (
            <p className="text-red-500">{outputFormatError}</p>
          )}
        </div>

        <div className="mb-4">
          <label className="mb-1 block">Constraint</label>
          <input
            type="text"
            name=""
            id=""
        ...
        <div className="mb-4">
          ...
          />
          {explanationError && (
            <p className="text-red-500">{explanationError}</p>
          )}
        </div>
        <label className="mb-1 block">Samples</label>
        <div className="flex gap-4">
          <div className=" w-full">
            {sampleInputs.map((input, index) => (
              <div key={index} className="flex w-full gap-x-6">
                <div className="w-full">
                  {index === 0 && <label className="mb-1 block">Input</label>}
                  <input
                    type="text"
                    value={input}
                    onChange={(e) =>
                  ...
                  />
                  {sampleInputError && (
                    <p className="text-red-500">{sampleInputError}</p>
                  )}
                </div>

                <div className="w-full">
                  {index === 0 && <label className="mb-1 block">Output</label>}
                  <input
                    type="text"
                  ...
                  />
                  {sampleOutputError && (
                    <p className="text-red-500">{sampleOutputError}</p>
                  )}
                </div>
              </div>
            ))}
          </div>
          <div className="mr-10 mt-10 flex flex-row gap-2 text-white">
            <button
              type="button"
              onClick={handleAddSample}
              className="h-fit rounded-sm bg-black px-4 hover:opacity-70"
            >
              +
            </button>
            <button
        ...
        <div className="flex justify-end ">
          <Button
            onPress={() => {
              editedProblemsData.id
                ? handleEditProblem()
                : handleCreateProblem();
            }}
            className="mr-8 mt-6 rounded-md bg-black px-5 py-2 text-white hover:opacity-60"
          >
            Next
          </Button>
        </div>
