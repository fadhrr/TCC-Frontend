...
const AdminProblemsLayout = ({url_create, url_edit}:any) => {
    ...
    const removeProblem = async (problemId: any)=>{
      try {
         const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/problems/${problemId}`, {
             method: 'DELETE',
             headers: {
                            ...
                            <tr key={index} className={index%2 === 0? 'bg-white' : 'bg-[#EDEDED]'}>
                                ...
                                <td className="border-y-2 border-r-2 border-black">
                                    <div className="flex space-x-3">
                                        <Link href={`${url_edit}/problems/edit/${item.id}`}>
