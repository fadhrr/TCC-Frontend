...
const AdminUserLayout = () => {
  ...
  const handleEditUser = async () => {
    try {
      ...
      if (response.status === 200) {
        ...
        setData((prevData) => prevData.map((user) => (user.id === editingUserId ? { ...user, ...editedUserData } : user)));
      } else {
  ...
  //     const rolesOrder = { 'Assistant': 1, 'Admin': 2, "User": 3};
  //     return rolesOrder[a.role] - rolesOrder[b.role];
  // });

  const filteredData = selectedRole !== 'default' ? data.filter((item) => item.role === selectedRole) : data;
            ...
            <>
              <div className="bg-white p-4 rounded-lg shadow-md">
                ...
                <ModalBody>
                  ...
                  <Input autoFocus placeholder="Enter your name" variant="bordered" value={editedUserData.name} onChange={(e) => setEditedUserData({ ...editedUserData, name: e.target.value })} />
                  <label htmlFor="">NIM</label>
                  <Input autoFocus placeholder="Enter your NIM" variant="bordered" value={editedUserData.nim} onChange={(e) => setEditedUserData({ ...editedUserData, nim: e.target.value })} />
                  <label htmlFor="">Email</label>
                  <Input autoFocus placeholder="Enter your email" type="email" variant="bordered" value={editedUserData.email} onChange={(e) => setEditedUserData({ ...editedUserData, email: e.target.value })} />
                  <label htmlFor="">Score</label>
                  <Input autoFocus placeholder="Enter your Score" variant="bordered" value={editedUserData.score} onChange={(e) => setEditedUserData({ ...editedUserData, score: e.target.value })} />
                </ModalBody>
                <ModalFooter>
                  ...
                  <Button
                    ...
                    className="px-3 py-1 rounded-sm hover:opacity-60"
                  >
                    Submit
                  </Button>
                </ModalFooter>
            ...
            {filteredData
              ...
              .map((user, index) => (
                <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-[#EDEDED]'}>
                  <td className={`border-y-2 border-s-2 border-black h-10 pl-3`}>{index + 1}</td>
                  <td className={`border-y-2 border-black`}>{user.name}</td>
                  <td className={`border-y-2 border-black`}>{user.role}</td>
