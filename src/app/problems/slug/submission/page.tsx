  ...
  return (
    <div className="w-full pt-[80px] my-8">
      <div className="flex justify-center">
        <button className={`${currentTab === 1 ? 'border-2 border-black text-md py-1 px-2 rounded-l-full bg-[#fae74d]' : 'border-2 border-black py-1 px-2 rounded-l-full hover:bg-[#fae74d]'}`} onClick={() => toogleTab(1)}>
          My Submision
        </button>
        {/* <span className='border-l-2 border-black'></span> */}
        <button className={`${currentTab === 2 ? 'border-2 border-black  text-md border-l-0 py-1 px-2 rounded-r-full bg-[#fae74d]' : 'border-2 border-black border-l-0 py-1 px-2 rounded-r-full hover:bg-[#fae74d]'}`} onClick={() => toogleTab(2)}>
          All Submissions
        </button>
      </div>
      ...
      <div className={`${currentTab === 1 ? '' : 'hidden'}`}>
              ...
              <tbody>
                {result.map((item, index) => (
                  <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-[#EDEDED]'}>
                    <td className={`border-y-2 border-s-2 border-black h-10 pl-3`}>{item.id}</td>
                    ...
                    <td className={`border-y-2 border-r-2 border-black`}>{item.score}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div>
          ...
          <object className="mt-4 border-2 border-black w-full" data="https://media.geeksforgeeks.org/wp-content/cdn-uploads/20210101201653/PDF.pdf" height="720"></object>
        </div>
      </div>
          ...
          <div className="table-wrapper block w-full overflow-x-scroll">
            <table className="items-center bg-transparent w-full border-separate border-spacing-y-3">
              ...
              <tbody>
                {data.map((item, index) => (
                  <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-[#EDEDED]'}>
                    <td className={` border-y-2 px-6 align-middle border-s-2 border-black h-10 pl-3`}>{item.id}</td>
                    <td className={`border-y-2 px-6 align-middle border-black`}>{item.user}</td>
                    <td className={`border-y-2 px-6 align-middle border-black`}>{item.archive}</td>
                    <td className={`border-y-2 px-6 align-middle border-black`}>{item.problems}</td>
                    <td className={`border-y-2 px-6 align-middle border-black`}>{item.lang}</td>
                    <td className={`border-y-2 px-6 align-middle border-black`}>{item.verdict}</td>
                    <td className={`border-y-2 px-6 align-middle border-black`}>{item.pts}</td>
                    <td className={`border-y-2 px-6 align-middle border-e-2 border-black`}>{item.time}</td>
                  </tr>
                ))}
              </tbody>
            </table>
